...
void RenderView(size_t whichEye,                                                //< Which eye are we rendering?
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(-overWidth / 2, overWidth / 2, -overHeight / 2, overHeight / 2, -1000, 1000);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
...
void Usage(const std::string& name) {
    std::cerr << "Usage: " << name << " [-eye right|left] (default is right)"
              ...
              << " (default auto-compute based on ranges seen)" << std::endl
...
// Set up a handler to cause us to exit cleanly.
    ...
    convert_to_normalized_and_meters(mapping, toMeters, depth, left, bottom, right, top, useFieldAngles);
    XY forward;
    forward.x = -1e5; // Start out off-screen
    forward.y = -1e5; // Start out off-screen
    for (size_t i = 0; i < mapping.size(); i++) {
        if ((mapping[i].xyLatLong.latitude == 0) && (mapping[i].xyLatLong.longitude == 0)) {
    ...
    while (!quit) {
        ...
        for (size_t i = 0; i < renderInfo.size(); i++) {
            RenderView(i, displayConfiguration, renderManagerConfig, renderInfo[i], frameBuffer,
                       colorBuffers[i].OpenGL->colorBufferName, depthBuffers[i], leftForward, rightForward);
        }
