...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (std::string("-mm") == argv[i]) {
            ...
            case 1:
            default:
                std::cerr << "Error: Expected no non-flag parameters, got " << argv[i] << std::endl;
                Usage(argv[0]);
            }
        }
    }
    ...
    if (g_verbose) {
        std::cerr << "Left, bottom, right, top = " << left << ", " << bottom << ", " << right << ", " << top
                  << std::endl;
    }
    ...
    for (auto& mapping : mappings) {
        ...
        // locations.
        convert_to_normalized_and_meters(leftMapping, toMeters, depth, leftScreenLeft, leftScreenBottom,
                                         leftScreenRight, leftScreenTop, useFieldAngles);
        convert_to_normalized_and_meters(rightMapping, toMeters, depth, rightScreenLeft, rightScreenBottom,
                                         rightScreenRight, rightScreenTop, useFieldAngles);
    ...
    if (g_verbose) {
        std::cerr << "Left screen L B R T: " << leftScreenLeft << ", " << leftScreenBottom << ", " << leftScreenRight
    ...
    for (size_t i = 0; i < mappings.size(); i++) {
        ...
        if (leftMesh.size() != leftMapping.size()) {
            std::cerr << "Error: Left mesh size " << leftMesh.size() << " does not match mapping size"
                      << leftMapping.size() << std::endl;
            return 4;
        }
        ...
        if (rightMesh.size() != rightMapping.size()) {
            std::cerr << "Error: Right mesh size " << rightMesh.size() << " does not match mapping size"
                      << rightMapping.size() << std::endl;
            return 6;
        }
...
static bool small(double d) { return fabs(d) <= 1e-5; }
...
static int testAlgorithms() {
    ...
    for (std::size_t entry = 0; entry < mesh.size(); entry++) {
        ...
        if (!small(mesh[entry][outIndex][0] - mapping[entry].xyLatLong.x)) {
            std::cerr << "testAlgorithms(): X normalized mesh mismatch for element: " << entry << " (got "
                      << mesh[entry][outIndex][0] << ", expected " << mapping[entry].xyLatLong.x << ")" << std::endl;
            return 400 + entry;
        }
        if (!small(mesh[entry][outIndex][1] - mapping[entry].xyLatLong.y)) {
            std::cerr << "testAlgorithms(): Y normalized mesh mismatch for element: " << entry << " (got "
                      << mesh[entry][outIndex][1] << ", expected " << mapping[entry].xyLatLong.y << ")" << std::endl;
            return 500 + entry;
        }
    }
    ...
    for (std::size_t entry = 0; entry < rmesh.size(); entry++) {
        ...
        if (!small(rmesh[entry][outIndex][1] - rmapping[entry].xyLatLong.y)) {
            std::cerr << "testAlgorithms(): Rotated Y normalized mesh mismatch for element: " << entry << " (got "
                      << rmesh[entry][outIndex][1] << ", expected " << rmapping[entry].xyLatLong.y << ")" << std::endl;
            return 900 + entry;
        }
    }
    ...
    dExpectedXIn.push_back(0.1);
    dExpectedYIn.push_back(0.1);
    dExpectedXOut.push_back(0.0);
    dExpectedYOut.push_back(0.0);
    Mapping dp2(XYLatLong(0.9, 0.1, -phi, theta), XYZ(1, -1, -1));
    dExpectedXIn.push_back(0.9);
    dExpectedYIn.push_back(0.1);
    dExpectedXOut.push_back(1.0);
    dExpectedYOut.push_back(0.0);
    Mapping dp3(XYLatLong(0.9, 0.9, phi, theta), XYZ(1, 1, -1));
    dExpectedXIn.push_back(0.9);
    dExpectedYIn.push_back(0.9);
    dExpectedXOut.push_back(1.0);
    dExpectedYOut.push_back(1.0);
    ...
    if (!convert_to_normalized_and_meters(ndmapping, 1, depth, 0, 0, 1, 1)) {
    ...
    for (std::size_t entry = 0; entry < ndmapping.size(); entry++) {
        ...
        if (!small(ndmapping[entry].xyz.x - dmapping[entry].xyz.x)) {
            std::cerr << "testAlgorithms(): Distorted X coord mismatch for element: " << entry << " (found "
                      << ndmapping[entry].xyz.x << ", expected " << dmapping[entry].xyz.x << ")" << std::endl;
            return 1100 + 3 * entry;
        }
        if (!small(ndmapping[entry].xyz.y - dmapping[entry].xyz.y)) {
            std::cerr << "testAlgorithms(): Distorted Y coord mismatch for element: " << entry << " (found "
                      << ndmapping[entry].xyz.y << ", expected " << dmapping[entry].xyz.y << ")" << std::endl;
            return 1101 + 3 * entry;
        }
        if (!small(ndmapping[entry].xyz.z - dmapping[entry].xyz.z)) {
            std::cerr << "testAlgorithms(): Distorted Z coord mismatch for element: " << entry << " (found "
                      << ndmapping[entry].xyz.z << ", expected " << dmapping[entry].xyz.z << ")" << std::endl;
            return 1102 + 3 * entry;
        }

        if (!small(ndmapping[entry].xyLatLong.x - dExpectedXIn[entry])) {
            std::cerr << "testAlgorithms(): Distorted X coord mismatch for element: " << entry << " (found "
                      << ndmapping[entry].xyLatLong.x << ", expected " << dExpectedXIn[entry] << ")" << std::endl;
            return 1103 + 3 * entry;
        }
        if (!small(ndmapping[entry].xyLatLong.y - dExpectedYIn[entry])) {
            std::cerr << "testAlgorithms(): Distorted Y coord mismatch for element: " << entry << " (found "
                      << ndmapping[entry].xyLatLong.y << ", expected " << dExpectedYIn[entry] << ")" << std::endl;
            return 1104 + 3 * entry;
        }
    }
    ...
    if (!small(dscreen.overlapPercent - 100)) {
        std::cerr << "testAlgorithms(): Distorted overlap percent not near 100: " << dscreen.overlapPercent
                  << std::endl;
        return 1203;
    }
    ...
    for (std::size_t entry = 0; entry < dmesh.size(); entry++) {
        ...
        if (!small(dmesh[entry][outIndex][0] - dExpectedXOut[entry])) {
            std::cerr << "testAlgorithms(): Distorted X normalized mesh mismatch for element: " << entry << " (found "
                      << dmesh[entry][outIndex][0] << ", expected " << dExpectedXOut[entry] << ")" << std::endl;
            return 1400 + entry;
        }
        if (!small(dmesh[entry][outIndex][1] - dExpectedYOut[entry])) {
            std::cerr << "testAlgorithms(): Distorted Y normalized mesh mismatch for element: " << entry << " (found "
                      << dmesh[entry][outIndex][1] << ", expected " << dExpectedYOut[entry] << ")" << std::endl;
            return 1500 + entry;
        }
    }
