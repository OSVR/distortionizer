...
// that we need.
static osvr::clientkit::ClientContext context("com.osvr.renderManager.distortionCorrectRenderManager");
...
void prevParam(void* /*userdata*/, const OSVR_TimeValue* /*timestamp*/, const OSVR_ButtonReport* report) {
    if (report->state == 1) {
        activeParam--;
        if (activeParam < 0) {
...
void nextParam(void* /*userdata*/, const OSVR_TimeValue* /*timestamp*/, const OSVR_ButtonReport* report) {
    if (report->state == 1) {
        activeParam++;
...
void setParams(void* userdata, const OSVR_TimeValue* /*timestamp*/, const OSVR_ButtonReport* report) {
    OSVRDisplayConfiguration* displayConfiguration = reinterpret_cast<OSVRDisplayConfiguration*>(userdata);
    ...
    if (report->state == 1) {
        ...
        distortionLeft.m_distortionCOP[0] = static_cast<float>(displayConfiguration->getEyes()[0].m_CenterProjX);
        distortionLeft.m_distortionCOP[1] = static_cast<float>(displayConfiguration->getEyes()[0].m_CenterProjY);

        osvr::renderkit::DistortionParameters distortionRight;
        distortionRight = distortionLeft;
        distortionRight.m_distortionCOP[0] = static_cast<float>(displayConfiguration->getEyes()[1].m_CenterProjX);
        distortionRight.m_distortionCOP[1] = static_cast<float>(displayConfiguration->getEyes()[1].m_CenterProjY);
        ...
        render->UpdateDistortionMeshes(osvr::renderkit::DistortionMeshType::SQUARE, distortionParams);
...
void resetParams(void* userdata, const OSVR_TimeValue* timestamp, const OSVR_ButtonReport* report) {
    if (report->state == 1) {
...
void RenderView(size_t whichEye,                                                //< Which eye are we rendering?
    ...
    glViewport(0, 0, static_cast<GLsizei>(renderInfo.viewport.width), static_cast<GLsizei>(renderInfo.viewport.height));
    ...
    glBegin(GL_LINES);
    if (whichEye == 1)
        for (double ofs = -1; ofs <= 1; ofs += 0.05) {
...
int main(int argc, char* argv[]) {
    ...
    do {
        ...
        if (elapsed.count() >= 1) {
            std::cerr << "Waiting to get Display from server..." << std::endl;
            start = end;
        }
    ...
    if (renderManagerConfig == nullptr) {
        std::cerr << "Could not parse /render_manager_parameters string from server (NULL pipelineconfig)."
                  << std::endl;
        return 101;
    }
    ...
    // "7" is pressed.
    osvr::clientkit::Interface button8 = context.getInterface("/controller/8");
    button8.registerCallback(&setParams, &displayConfiguration);

    osvr::clientkit::Interface button7 = context.getInterface("/controller/7");
    button7.registerCallback(&resetParams, &displayConfiguration);

    osvr::clientkit::Interface button5 = context.getInterface("/controller/5");
    button5.registerCallback(&prevParam, nullptr);
    osvr::clientkit::Interface button6 = context.getInterface("/controller/6");
    ...
    osvr::clientkit::Interface analogTrigger = context.getInterface("/controller/trigger");
...
// Set up a handler to cause us to exit cleanly.
    ...
    */
    ...
    while (!quit) {
        ...
        if (triggerValue != 0) {
            if (activeParam < params.size()) {
                double changeScale = 1.0;
                pow(1.5, activeParam);
                params[activeParam] -= static_cast<float>(elapsed_sec.count() * triggerValue / (10 * changeScale));
            }
        }
        ...
        for (size_t i = 0; i < renderInfo.size(); i++) {
            RenderView(i, displayConfiguration, renderManagerConfig, renderInfo[i], frameBuffer,
                       colorBuffers[i].OpenGL->colorBufferName, depthBuffers[i], spheres);
        }
