...
void RenderView(size_t whichEye,                                                //< Which eye are we rendering?
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(-width / 2 * overFill, width / 2 * overFill, -height / 2 * overFill, height / 2 * overFill, 100, -100);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
...
void Usage(std::string name) {
    std::cerr << "Usage: " << name << " [color (one of red, green, blue, white, cyan, magenta, yellow)]" << std::endl;
    exit(-1);
}
    ...
    while (!quit) {
        ...
        for (size_t i = 0; i < renderInfo.size(); i++) {
            RenderView(i, displayConfiguration, renderManagerConfig, renderInfo[i], frameBuffer,
                       colorBuffers[i].OpenGL->colorBufferName, depthBuffers[i], xSphere, ySphere, spheres, color,
                       sphereSpace / 4);
        }
